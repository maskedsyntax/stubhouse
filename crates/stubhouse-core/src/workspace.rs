...
mod tests {
    ...
    fn save_then_list_and_load() {
        ...
        let id = ws
            .save_request("users", "get-users", &sample_def("Get Users"))
            .unwrap();
    ...
    fn save_rejects_path_traversal_in_collection() {
        ...
        let ws = Workspace::init(dir.path(), "demo").unwrap();
        let err = ws
            .save_request("../evil", "x", &sample_def("x"))
            .unwrap_err();
    ...
    fn list_requests_ignores_non_yaml_files() {
        let dir = TempDir::new().unwrap();
        let ws = Workspace::init(dir.path(), "demo").unwrap();
        ws.save_request("users", "get-users", &sample_def("Get Users"))
            .unwrap();
